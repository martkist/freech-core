...
RUN /bin/bash -c 'for LIB in $(find $INSTALLPREFIX -name \*.a); \
        do rm -rf $TEMPDIR && mkdir $TEMPDIR && cd $TEMPDIR; \
        $HOST-ar xv $LIB | cut -b5- > /tmp/list.txt; \
        rm $LIB; \d
        $HOST-ar crsD $LIB $(cat /tmp/list.txt); \
    done'

WORKDIR $INSTALLPREFIX
RUN find | sort | zip -X@ $OUTDIR/boost-win$BITS-1.71.0.zip